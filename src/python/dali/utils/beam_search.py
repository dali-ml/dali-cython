...
def beam_search(initial_state,
    ...
    iterations = 0
    results = [
        Beam([], dali.Array.zeros([]), initial_state)
    ]
    ...
    while max_sequence_length is None or iterations < max_sequence_length:
        ...
        for beam in results:
            if check_eos(beam):
                ...
                sorted_candidates = dali.array.argsort(scores).eval()
