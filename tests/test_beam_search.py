...
from dali.utils import beam_search, Beam
...
class BeamSearchTests(unittest.TestCase):
    def test_letters(self):
        ...
        res_aa = Beam([0,0], dali.Array(math.log(0.6 * 0.55), dtype=dali.float32), "aa")
        res_ab = Beam([0,1], dali.Array(math.log(0.6 * 0.45), dtype=dali.float32), "ab")
        res_ba = Beam([1,0], dali.Array(math.log(0.4 * 0.99), dtype=dali.float32), "ba")
        res_bb = Beam([1,1], dali.Array(math.log(0.4 * 0.11), dtype=dali.float32), "bb")

        initial_state = "";
        def candidate_scores(state):
            ret = dali.Array([
                math.log(choices[state + "a"]),
                math.log(choices[state + "b"]),
            ], dtype=dali.float32)
            return ret
        ...
        def beams_equal(b1, b2):
            return (b1.solution == b2.solution and
                    np.allclose(b1.score.tonumpy(), b2.score.tonumpy()) and
                    b1.state == b2.state)
